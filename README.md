...
### 2024.5.8 更新：完成了Job模式的主要开发任务

#### 更新内容

1. 修改了README文件
2. 完善了Job模式，现在它既可以按照预设指令队列运行，也可以按照遥控app输入的指令队列运行
...
2. 打开蓝牙遥控app，将相关按键名称修改得和截图一样；将相关参数条名称和数值范围和起始值也修改得和截图一样。按钮的位置和发送的数据一一对应，因此要特别注意按钮的位置是不是对的。按钮发送的数据可以在”主页”的debug栏看见，就是一堆字母。*（截图在哪？下次再发github好了）*
...
5. 用手固定小车，按下启动键，小车会停止运作且不保持平衡。（Flag_Stop = 1，1为停止状态）
...
标识模式的核心变量是Flag_avoid和Flag_follow。如果Flag_avoid==1就是避障模式，如果Flag_follow==1就是跟随模式。因此在这里我加了一个Flag_swing表示摇摆模式。我还增加了Flag_job表示指令队列模式。
...
在最新的更新中，Job模式的主要代码逻辑也和Swing模式的代码处于大概一致的地方，可以非常容易的发现。

### WHEELTEC/show.c

这是用于设计OLED屏幕的文件。

我修改了里面的部分内容以在OLED上显示sin和Pointer当前的数值。

已经修正了普通模式下Angle也会被替换成sin的问题。

### Core/Inc/main.h

我在里面添加了相关的变量声明以令相关行为生效。

### BSPcode/usart3.c

这里包括了蓝牙遥控的数据接收和解读逻辑。在里面搜索job_pointer就可以找到发生了修改的地方。

自定义按键的本质就是abcdefghijk等字母，这里是根据收到的字母ascii值执行对应的逻辑。

按键实现的清空指令队列和添加行动等逻辑都在这里实现。