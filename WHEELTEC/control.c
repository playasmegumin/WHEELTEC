...
u8 sin100_counter = 0;
int job_counter = 0, job_action = 0, job_pointer = 0;
int job_list[20] = {0,0,0,1,0,2,0,1,0,2,0,1,0,2,0,1,0};
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) // 用于处理来自MPU6050的时钟中断，这里是实际上的主函数
{
	...
	//	if(Flag_swing==1 && Flag_Quatre == 4) 				 // 本来想写这个，但为了方便调整摇摆周期还是加了个Swing_period，在其声明处赋值。
		...
		if(Flag_job == 1 && Flag_Stop == 0)
		{
			...
			switch(job_action) {
			case 1:
				if(sin100[0] < 0.25) sin100[0] += 0.001;
				if(sin100[0] > 0.25) sin100[0]  = 0.25;
				Flag_Left = 0;
				Flag_Right = 0;
				break;
			case 2:
			...
			case 0:
			default:
				if(sin100[0] > 0) sin100[0] -= 0.001;		// 这里默认将两个Flag置零，可能会影响到App的控制
