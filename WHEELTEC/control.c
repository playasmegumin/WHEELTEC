...
u8 sin100_counter = 0;
int job_counter = 0, job_action = 0, job_pointer = 0;				// job_pointer: 预设队列指针 / job_action: 当前时刻行动 / job_counter: 当前行动已进行进度(200一秒那个玩意)
int job_usart_pointer = 0;											// job_usart_pointer: 遥控队列指针
int job_list[60] = {0,0,0,1,0,2,0,1,0,2,0,1,0,2,0,1,0};				// 0~19: 预设队列 / 20~59: 遥控队列
int job_dir[4][3] = {{0,0,400},{1,1,800},{-1,+1,150},{+1,-1,150}}; 	// 停止 / 前进 / 左转 / 右转
#define JOB_SPEED 0.45		// 这是指令模式里“前进”行动的速度，范围是-1 ~ +1, 可以考虑0.5或者更快，感觉现在这个速度有点慢了
#define JOB_SPEEDUP 0.005 	// 上面这个速度的加速度，这样v-t图是一个梯形
#define JOB_APP_LIST_BEGIN 20
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) // 用于处理来自MPU6050的时钟中断，这里是实际上的主函数
{
	...
	//	if(Flag_swing==1 && Flag_Quatre == 4) 				 // 本来想写这个，但为了方便调整摇摆周期还是加了个Swing_period，在其声明处赋值。
		...
		if(Flag_job == 1 && Flag_Stop == 0)
		{
			...
			switch(job_action) {
			case 1:												// 前进，设置速度为JOB_SPEED，加速度为JOB_SPEEDUP
																// 设置加速度是为了让加速更加平滑，避免剧烈变化带来小车前后摇晃
				if(sin100[0] < JOB_SPEED) sin100[0] += JOB_SPEEDUP;
				if(sin100[0] > JOB_SPEED) sin100[0]  = JOB_SPEED;
				Flag_Left = 0;
				Flag_Right = 0;
				break;
			case 2:												// 左转，设置速度为0，并借用原有的左转代码
				sin100[0] = 0;
				Flag_Left = 1;
				Flag_Right = 0;
				break;
			case 3:												// 右转，设置速度为0，并借用原有的右转代码
				sin100[0] = 0;
				Flag_Left = 0;
				Flag_Right = 1;
				break;
			case 4:												// 暂停（遥控限定），如果现在是action 4，job_pointer不应该动。
				sin100[0] = 0;
				Flag_Left = 0;
				Flag_Right = 0;
				break;
			case 0:
			default:
				if(sin100[0] > 0) sin100[0] -= JOB_SPEEDUP;		// 这里默认将两个Flag置零，可能会影响到App的控制
				if(sin100[0] < 0) sin100[0]  = 0;
				Flag_Left = 0;
				Flag_Right = 0;
				break;
			}
			...
			}else if(job_counter > job_dir[job_action][2])
			{
				if(job_pointer < 19){
					job_pointer++;
					job_counter = 0;
				...
				if(job_pointer > 19 && job_pointer < 59){
					job_pointer++;
					job_counter = 0;
				}
			...
			job_counter = 0, job_pointer = 0, sin100[0] = 0;
...
void Choose(int encoder_left,int encoder_right)
{
	...
	if(Flag_Stop==0)
		count = 0;
	// 一旦Flag_Stop就会进入Choose模式，由于任务模式结束后会自动set这个flag，导致每次执行完成自动进入默认模式，懒得改逻辑了，直接换一个默认模式
